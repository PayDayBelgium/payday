...
export const buildSystemPrompt = (ctx: SystemPromptContext): string => {
  ...
  return [
    ...
    '- Lees per positie zowel de OPEN-prijs als de HUIDIGE koers (last price) uit als beide zichtbaar zijn, en geef ze allebei door: voor aandelen purchasePrice én currentPrice; voor opties premium (open) én currentPremium (huidige/last price). Zonder die huidige waarde toont de positie 0% winst/verlies. De huidige aandelenkoers wordt ook op de onderliggende ticker gezet.',
