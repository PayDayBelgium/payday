...
const strategiesSlice = createSlice({
  ...
  reducers: {
    ...
    addPositionToStrategy: (state, action: PayloadAction<{ strategyId: string; positionId: string }>) => {
    ...
    removePositionFromStrategy: (state, action: PayloadAction<{ strategyId: string; positionId: string }>) => {
      const strategy = state.strategies.find(s => s.id === action.payload.strategyId);
      if (strategy) {
        strategy.positionIds = strategy.positionIds.filter(id => id !== action.payload.positionId);
        strategy.updatedAt = new Date().toISOString();
      }
    },

    // Set all positions for a strategy
    setStrategyPositions: (state, action: PayloadAction<{ strategyId: string; positionIds: string[] }>) => {
      const strategy = state.strategies.find(s => s.id === action.payload.strategyId);
      if (strategy) {
...
export const selectStrategiesByPortfolio = (portfolio: PortfolioName) =>
  createSelector(
    [selectAllStrategies],
    (strategies) => strategies.filter(s => s.portfolio === portfolio)
  );

export const selectStrategyById = (id: string) =>
  createSelector(
    [selectAllStrategies],
    (strategies) => strategies.find(s => s.id === id)
  );

export const selectStrategyByPositionId = (positionId: string) =>
  createSelector(
    [selectAllStrategies],
    (strategies) => strategies.find(s => s.positionIds.includes(positionId))
  );
...
export const selectStrategyRulesByPortfolioAndType = (portfolio: PortfolioName, strategyType: StrategyType) =>
  createSelector(
    [selectAllStrategyRules],
    (rules) => rules.filter(r => r.portfolio === portfolio && r.strategyType === strategyType)
  );

export const selectEnabledStrategyRules = createSelector(
  [selectAllStrategyRules],
...
export const selectStrategyRulesByPortfolio = (portfolio: PortfolioName) =>
  createSelector(
    [selectAllStrategyRules],
    (rules) => rules.filter(r => r.portfolio === portfolio)
  );
...
export const selectIsAlertDismissed = (alertId: string) =>
  createSelector(
    ...
    (dismissed) => dismissed.includes(alertId)
  );
