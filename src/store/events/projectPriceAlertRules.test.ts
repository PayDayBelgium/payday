...
describe('applyPriceAlertRuleEvent', () => {
  it('Created appends', () => {
    expect(
      applyPriceAlertRuleEvent([], ev('PriceAlertRuleCreated', { rule: rule('r1') })).map(
        (r) => r.id
      )
    ).toEqual(['r1']);
  });
  it('Updated replaces', () => {
    ...
    const next = applyPriceAlertRuleEvent(
      s,
      ev('PriceAlertRuleUpdated', { rule: { ...s[0], isActive: false } })
    );
  ...
  it('Toggled flips isActive', () => {
    const s = applyPriceAlertRuleEvent([], ev('PriceAlertRuleCreated', { rule: rule('r1') }));
    expect(applyPriceAlertRuleEvent(s, ev('PriceAlertRuleToggled', { id: 'r1' }))[0].isActive).toBe(
      false
    );
  });
});
