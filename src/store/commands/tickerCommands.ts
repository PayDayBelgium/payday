...
export const addTicker =
  (ticker: Ticker, timestamp: string) => (dispatch: AppDispatch) =>
    dispatch(
      commit([
        createEvent(
          'TickerAdded',
...
export const ensureTicker =
  (ticker: Ticker, timestamp: string) =>
  (dispatch: AppDispatch, getState: () => RootState) => {
    const symbolUp = ticker.symbol.toUpperCase();
    const exists = getState().tickers.tickers.some(
      (t) => t.symbol.toUpperCase() === symbolUp
    );
    if (exists) return;
    dispatch(
      commit([
...
export const updateTicker =
  (patch: Partial<Ticker> & { symbol: string }, timestamp: string) =>
...
export const renameTicker =
  (symbol: string, name: string, timestamp: string) => (dispatch: AppDispatch) =>
    dispatch(
      commit([
        createEvent('TickerRenamed', { symbol: symbol.toUpperCase(), name }, timestamp),
      ])
    );
...
export const removeTicker =
  (symbol: string, timestamp: string) => (dispatch: AppDispatch) =>
    dispatch(
      commit([createEvent('TickerRemoved', { symbol: symbol.toUpperCase() }, timestamp)])
    );
