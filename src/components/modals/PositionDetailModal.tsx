...
export const PositionDetailModal: React.FC<PositionDetailModalProps> = ({
  ...
  const profitPercent =
    option && option.costBasis !== 0 ? (nominalProfit / Math.abs(option.costBasis)) * 100 : 0;
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white dark:bg-trading-dark-800 rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-white dark:bg-trading-dark-800 border-b border-surface-line dark:border-trading-dark-600 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              {option?.name && (
                <p className="text-sm text-ink-600 dark:text-ink-400">{option.name}</p>
              )}
              {(position.type === 'stock' || position.type === 'etf') &&
                'name' in position &&
                position.name && (
                  <p className="text-sm text-ink-600 dark:text-ink-400">{position.name}</p>
                )}
            </div>
            {option && (
        ...
        {/* Tab Headers */}
        {option && (
          <div className="flex bg-surface dark:bg-trading-dark-800 border-b border-surface-line dark:border-trading-dark-600">
            <button
              type="button"
              onClick={() => setActiveTab('overview')}
              className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium transition-colors border-b-2 ${
                activeTab === 'overview'
                  ? 'bg-white dark:bg-trading-dark-700 text-primary-700 dark:text-primary-300 border-primary-700 dark:border-primary-400'
                  : 'text-ink-600 dark:text-ink-400 hover:text-ink-900 dark:hover:text-ink-200 border-transparent'
              }`}
            >
        ...
        <div className="p-6 space-y-6 min-h-[600px]">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-20 px-2 py-0.5 text-lg font-bold bg-white dark:bg-trading-dark-700 border border-primary-300 dark:border-primary-700 rounded text-primary-900 dark:text-primary-300"
                          placeholder="0,00"
                        />
                        <p className="text-sm text-primary-700 dark:text-primary-300 mt-1">
                          {t('modalsA.total', {
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                          <div>
                            ...
                            <input
                              ...
                              className="w-28 px-3 py-2 text-lg font-bold bg-white dark:bg-trading-dark-700 border border-primary-300 dark:border-primary-700 rounded text-primary-900 dark:text-primary-300"
                              placeholder="0,00"
                            />
                          </div>
          ...
          {activeTab === 'pnl' && option && (
            <PnLCurve
              ...
              contracts={option.contracts}
              actualCurrentPrice={
                currentPrice ? parseNumberInput(currentPrice) * option.strike : undefined
              }
              currency={currency}
            />
          )}
        ...
        {/* Footer */}
        <div className="sticky bottom-0 bg-surface dark:bg-trading-dark-900 border-t border-surface-line dark:border-trading-dark-600 px-6 py-4">
