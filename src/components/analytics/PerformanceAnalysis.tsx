...
const startOfPreset = (preset: Exclude<PeriodPreset, 'custom'>): Date => {
  ...
  switch (preset) {
    ...
    case 'quarter': return new Date(now.getTime() - 90 * 86400_000);
...
export const PerformanceAnalysis: React.FC = () => {
  ...
  const [customFrom, setCustomFrom] = useState<string>(toIso(new Date(Date.now() - 30 * 86400_000)));
  ...
  const stats = useMemo(() => {
    ...
    const best  = periodTrades.reduce<Trade | null>((b, t) => (!b || t.realizedPnL > b.realizedPnL ? t : b), null);
    const worst = periodTrades.reduce<Trade | null>((w, t) => (!w || t.realizedPnL < w.realizedPnL ? t : w), null);
    ...
    return {
      totalPnL, totalCost, totalFees, pnlPercent, deltaPnL,
      wins: wins.length, losses: losses.length, breakevens: breakevens.length,
      grossProfit, grossLoss, winRate, avgPnL, avgWin, avgLoss, profitFactor,
      best, worst, totalContracts,
    };
  ...
  const chartData = useMemo(() => {
    ...
    return sorted.map((t) => {
      ...
      return {
        ...
        value: display === 'nominal' ? running : (stats.totalCost > 0 ? (running / stats.totalCost) * 100 : 0),
        pnl:   t.realizedPnL,
      };
    });
  ...
  const fmt = (v: number) => display === 'nominal' ? formatCurrency(v) : formatPercent(stats.totalCost > 0 ? (v / stats.totalCost) * 100 : 0);

  const kpis: KPI[] = [
    {
      label: 'Netto P&L',
      value: display === 'nominal' ? formatCurrency(stats.totalPnL) : formatPercent(stats.pnlPercent),
      hint:  stats.deltaPnL >= 0
    ...
    },
    {
      ...
      hint:  stats.profitFactor >= 2 ? 'Sterk' : stats.profitFactor >= 1 ? 'Boven break-even' : 'Onder break-even',
    ...
    {
      label: 'Bruto winst',
      value: display === 'nominal' ? formatCurrency(stats.grossProfit) : formatPercent(stats.totalCost > 0 ? (stats.grossProfit / stats.totalCost) * 100 : 0),
    ...
    {
      label: 'Bruto verlies',
      value: display === 'nominal' ? formatCurrency(stats.grossLoss) : formatPercent(stats.totalCost > 0 ? (stats.grossLoss / stats.totalCost) * 100 : 0),
      hint:  stats.losses > 0 ? `Gem. ${formatCompact(stats.avgLoss)} per loss` : 'Geen verliezende trades',
      tone:  'negative',
      icon:  <TrendingDown className="w-4 h-4" />,
    },
    {
      label: 'Gem. P&L / trade',
      value: display === 'nominal' ? formatCurrency(stats.avgPnL) : formatPercent(periodTrades.length > 0 && stats.totalCost > 0 ? (stats.avgPnL / (stats.totalCost / periodTrades.length)) * 100 : 0),
    ...
    },
    {
      ...
      hint:  stats.totalPnL !== 0 ? `${((stats.totalFees / Math.abs(stats.totalPnL)) * 100).toFixed(1)}% van P&L` : '—',
      tone:  'neutral',
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1 p-1 rounded-md border border-[var(--line)] bg-white dark:bg-trading-dark-800">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-colors ${
                display === 'nominal' ? 'bg-ink-900 text-white dark:bg-ink-100 dark:text-ink-900' : 'text-ink-700 dark:text-ink-300'
              }`}
            >
              € Nominaal
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-colors ${
                display === 'percent' ? 'bg-ink-900 text-white dark:bg-ink-100 dark:text-ink-900' : 'text-ink-700 dark:text-ink-300'
              }`}
            >
              % Procent
            </button>
      ...
      <div className="flex items-center gap-2 text-xs text-ink-500 dark:text-ink-400">
        ...
        <span>{toIso(fromDate)} → {toIso(toDate)}</span>
      </div>

      {!hasData ? (
        <div className="surface-card p-12 text-center">
          <Activity className="w-12 h-12 mx-auto mb-3 text-ink-300" strokeWidth={1.5} />
          <h3 className="text-base font-semibold text-ink-900 dark:text-white mb-1">Geen trades in deze periode</h3>
            ...
            {kpis.map((kpi) => (
              <div key={kpi.label} className="bg-white dark:bg-trading-dark-800 p-4">
                <div className="flex items-center gap-2 text-ink-500 dark:text-ink-400 mb-2">
                  <span className={
                    kpi.tone === 'positive' ? 'text-positive-600 dark:text-positive-500' :
                    kpi.tone === 'negative' ? 'text-negative-600 dark:text-negative-500' :
                    'text-ink-500 dark:text-ink-400'
                  }>{kpi.icon}</span>
                  <p className="text-[11px] uppercase tracking-[0.08em] font-semibold">{kpi.label}</p>
                </div>
                <p className={`text-xl font-semibold tabular-nums tracking-tight ${
                  kpi.tone === 'positive' ? 'text-positive-700 dark:text-positive-500' :
                  kpi.tone === 'negative' ? 'text-negative-700 dark:text-negative-500' :
                  'text-ink-900 dark:text-white'
                }`}>
                  {kpi.value}
                </p>
                {kpi.hint && (
                  <p className="text-[11px] text-ink-500 dark:text-ink-400 mt-1 leading-snug">{kpi.hint}</p>
                )}
              </div>
            ))}
          </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-right">
                  <p className={`text-base font-semibold tabular-nums ${
                    stats.totalPnL >= 0 ? 'text-positive-700 dark:text-positive-500' : 'text-negative-700 dark:text-negative-500'
                  }`}>
                    {display === 'nominal' ? formatCurrency(stats.totalPnL) : formatPercent(stats.pnlPercent)}
                  </p>
                  <p className="text-[11px] text-ink-500 dark:text-ink-400">{periodTrades.length} trade(s)</p>
                </div>
              </div>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={chartData} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <XAxis dataKey="date" tick={{ fontSize: 11, fill: '#8A99B0' }} axisLine={false} tickLine={false} minTickGap={32} />
                    <YAxis
                      tick={{ fontSize: 11, fill: '#8A99B0' }}
                      axisLine={false}
                      tickLine={false}
                      tickFormatter={(v) => display === 'nominal' ? formatCompact(v) : `${v.toFixed(1)}%`}
                      width={56}
                    />
                    <Tooltip
                      contentStyle={{ border: '1px solid #DCE4EE', borderRadius: 6, fontSize: 12 }}
                      formatter={(value: number) => [display === 'nominal' ? formatCurrency(value) : `${value.toFixed(2)}%`, 'Cumulatief']}
                    />
            ...
            <div className="surface-card p-5">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="font-semibold tabular-nums text-positive-700 dark:text-positive-500">{stats.wins}</span>
                  </div>
                  <div className="h-2.5 rounded-full bg-surface-subtle overflow-hidden">
                    <div className="h-full bg-positive-600 rounded-full" style={{ width: `${periodTrades.length > 0 ? (stats.wins / periodTrades.length) * 100 : 0}%` }} />
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between text-xs mb-1">
                    <span className="text-ink-500 dark:text-ink-400">Verliezers</span>
                    <span className="font-semibold tabular-nums text-negative-700 dark:text-negative-500">{stats.losses}</span>
                  </div>
                  <div className="h-2.5 rounded-full bg-surface-subtle overflow-hidden">
                    <div className="h-full bg-negative-600 rounded-full" style={{ width: `${periodTrades.length > 0 ? (stats.losses / periodTrades.length) * 100 : 0}%` }} />
                  </div>
                </div>
                {stats.breakevens > 0 && (
                  <div>
                    <div className="flex items-center justify-between text-xs mb-1">
                      ...
                      <span className="font-semibold tabular-nums text-ink-700 dark:text-ink-300">{stats.breakevens}</span>
                    </div>
                    <div className="h-2.5 rounded-full bg-surface-subtle overflow-hidden">
                      <div className="h-full bg-ink-400 rounded-full" style={{ width: `${(stats.breakevens / periodTrades.length) * 100}%` }} />
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="border-t border-[var(--line)] pt-4 mt-5 space-y-2.5 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold tabular-nums text-positive-700 dark:text-positive-500">{formatCurrency(stats.avgWin)}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-ink-500 dark:text-ink-400">Gem. verlies per loss</span>
                  <span className="font-semibold tabular-nums text-negative-700 dark:text-negative-500">{formatCurrency(stats.avgLoss)}</span>
                </div>
                <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                {stats.best && (
                  <div className="bg-white dark:bg-trading-dark-800 p-4">
                    <div className="flex items-center gap-2 text-positive-600 dark:text-positive-500 mb-2">
                      ...
                      <p className="text-[11px] uppercase tracking-[0.08em] font-semibold">Beste trade</p>
                    </div>
                    ...
                    <p className="text-xs text-ink-700 dark:text-ink-300 mt-1.5 font-medium">{stats.best.ticker}</p>
                ...
                {stats.worst && stats.worst !== stats.best && (
                  <div className="bg-white dark:bg-trading-dark-800 p-4">
                    <div className="flex items-center gap-2 text-negative-600 dark:text-negative-500 mb-2">
                      ...
                      <p className="text-[11px] uppercase tracking-[0.08em] font-semibold">Slechtste trade</p>
                    </div>
                    ...
                    <p className="text-xs text-ink-700 dark:text-ink-300 mt-1.5 font-medium">{stats.worst.ticker}</p>
                    <p className="text-[11px] text-ink-500 dark:text-ink-400">
            ...
            <div className="surface-card p-5">
              ...
              {strategyData.length > 0 ? (
                <div className="h-56">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={strategyData} layout="vertical" margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                      <CartesianGrid stroke="#E9EDF3" strokeDasharray="2 4" horizontal={false} />
                      <XAxis type="number" tick={{ fontSize: 11, fill: '#8A99B0' }} axisLine={false} tickLine={false} tickFormatter={formatCompact} />
                      <YAxis dataKey="strategy" type="category" tick={{ fontSize: 11, fill: '#5A6B82' }} axisLine={false} tickLine={false} width={110} />
                      <Tooltip
                        contentStyle={{ border: '1px solid #DCE4EE', borderRadius: 6, fontSize: 12 }}
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {[...periodTrades]
                    ...
                    .slice(0, 25)
                    .map((t) => (
                      <tr key={t.id} className="border-t border-[var(--line)] hover:bg-surface-subtle/50">
                        <td className="px-5 py-2.5 text-ink-700 dark:text-ink-300 tabular-nums">{t.closeDate.slice(0, 10)}</td>
                        <td className="px-5 py-2.5 font-semibold text-ink-900 dark:text-white">{t.ticker}</td>
                        <td className="px-5 py-2.5 text-ink-500 dark:text-ink-400">{t.strategy}</td>
                        <td className="px-5 py-2.5 text-right text-ink-700 dark:text-ink-300 tabular-nums">{t.quantity}</td>
                        <td className="px-5 py-2.5 text-right text-ink-700 dark:text-ink-300 tabular-nums">{formatCurrency(t.entryPrice)}</td>
                        <td className="px-5 py-2.5 text-right text-ink-700 dark:text-ink-300 tabular-nums">{formatCurrency(t.exitPrice)}</td>
                        <td className="px-5 py-2.5 text-right text-ink-500 dark:text-ink-400 tabular-nums">{formatCurrency((t.commission || 0) + (t.fees || 0))}</td>
                        <td className={`px-5 py-2.5 text-right tabular-nums font-semibold ${
                          t.realizedPnL > 0 ? 'text-positive-700 dark:text-positive-500' :
                          t.realizedPnL < 0 ? 'text-negative-700 dark:text-negative-500' :
