...
const VERDICT_CLS: Record<Verdict, string> = {
  excellent: 'text-positive-600',
  suitable: 'text-positive-600',
  mediocre: 'text-caution-600',
  unsuitable: 'text-negative-600',
};
...
export const OptionCheck: React.FC = () => {
  ...
  const STATUS_LABEL: Record<CriterionStatus, string> = {
    good: t('toolsPages.oc.statusGood'),
    ok: t('toolsPages.oc.statusOk'),
    bad: t('toolsPages.oc.statusBad'),
  };

  const VERDICT_LABEL: Record<Verdict, string> = {
    excellent: t('toolsPages.oc.verdictExcellent'),
    suitable: t('toolsPages.oc.verdictSuitable'),
    mediocre: t('toolsPages.oc.verdictMediocre'),
    unsuitable: t('toolsPages.oc.verdictUnsuitable'),
  };
  ...
  useEffect(() => {
    setPageTitle(t('toolsPages.oc.pageTitle'), t('toolsPages.oc.pageSubtitle'));
  }, [setPageTitle, t]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-2 rounded-md border border-caution-500/40 bg-caution-50 dark:bg-caution-600/10 px-3 py-2 text-xs text-caution-700 dark:text-caution-300 max-w-3xl">
        ...
        <span dangerouslySetInnerHTML={{ __html: t('toolsPages.oc.disclaimer') }} />
      </div>

      {/* Ticker selection / add */}
      <div className="surface-card p-5">
        <p className="text-xs font-semibold text-ink-500 mb-1.5">{t('toolsPages.oc.ticker')}</p>
        {isCreatingTicker ? (
          <div className="space-y-3">
            <input
              ...
              placeholder={t('toolsPages.oc.symbolPlaceholder')}
            />
            <input
              className={fieldClass}
              type="text"
              ...
              placeholder={t('toolsPages.oc.namePlaceholder')}
            />
            ...
            <div className="flex gap-2">
              ...
              >
                <Plus className="w-4 h-4" /> {t('toolsPages.add')}
              </button>
              <button
          ...
          <TickerSelector
            ...
            placeholder={t('toolsPages.oc.tickerSelectorPlaceholder')}
          />
        )}
      </div>
      ...
      {assessment ? (
          ...
          <div className="space-y-6">
            ...
            <div className="surface-card p-6">
              <div className="flex items-center justify-between mb-3">
                <p className="eyebrow">
                  {t('toolsPages.oc.verdict')} — {selected!.symbol}
                </p>
              ...
              <p
                className={`text-lg font-semibold tracking-tight ${VERDICT_CLS[assessment.result.verdict]}`}
              >
                {VERDICT_LABEL[assessment.result.verdict]}
              </p>
              <div
                ...
                aria-valuemin={0}
                aria-valuemax={100}
                aria-label={t('toolsPages.oc.suitabilityScore')}
              >
                <div
            ...
            <div className="surface-card p-5">
              <div className="flex items-center justify-between mb-3">
                <p className="eyebrow">{t('toolsPages.oc.optionFigures')}</p>
                <span className="text-[10px] font-semibold uppercase tracking-[0.08em] text-caution-600 bg-caution-50 dark:bg-caution-600/15 px-2 py-0.5 rounded-full">
                  {t('toolsPages.oc.simulated')}
                </span>
              </div>
              <dl className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3">
                {[
                  { label: t('toolsPages.oc.ivRank'), value: `${assessment.data.ivRank}/100` },
                  {
                    label: t('toolsPages.oc.openInterest'),
                    value: assessment.data.openInterest.toLocaleString('nl-NL'),
                  },
                  {
                    label: t('toolsPages.oc.volumePerDay'),
                    value: assessment.data.optionVolume.toLocaleString('nl-NL'),
                  },
                  {
                    label: t('toolsPages.oc.bidAskSpread'),
                    value: `${assessment.data.bidAskSpreadPct}%`,
                  },
                  {
                    label: t('toolsPages.oc.premiumAnnualized'),
                    value: `${assessment.data.annualizedPremiumPct}%`,
                  },
                  {
                    label: t('toolsPages.oc.earningsIn'),
                    value: `${assessment.data.daysToEarnings} d`,
                  },
                ].map((f) => (
                  <div key={f.label}>
        ...
        <p className="text-sm text-ink-400">{t('toolsPages.oc.chooseTicker')}</p>
      )}
    </div>
  );
};
