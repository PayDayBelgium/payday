...
import { TrendingUp, Plus, Info, ArrowRight, X, ListTodo, GraduationCap, Lightbulb } from 'lucide-react';
...
export const StocksETFsStrategy: React.FC = () => {
  ...
  const [activeRuleCategory, setActiveRuleCategory] = useState<'alert' | 'opportunity' | undefined>(undefined);
    ...
    if (saved) {
      ...
      const migratedRules = rules.map(rule => {
        ...
        if (rule.trigger === 'price_increase' || rule.trigger === 'profit_target' ||
            rule.trigger === 'time_based' || rule.trigger === 'volatility') {
  ...
  const stockPositions = useAppSelector(selectPositionsByPortfolioAndType(portfolio || '', 'stock')) as StockPosition[];
  const etfPositions = useAppSelector(selectPositionsByPortfolioAndType(portfolio || '', 'etf')) as StockPosition[];
  ...
  useEffect(() => {
    if (strategyRules.length > 0) {
      localStorage.setItem(`strategy-rules-stocks-etfs-${portfolio}`, JSON.stringify(strategyRules));
    }
        ...
        <>
          ...
          <div className={`grid grid-cols-1 gap-4 ${currentPortfolio?.hasOptions ? 'md:grid-cols-3' : 'md:grid-cols-2'}`}>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white mt-1">{totalPositions}</p>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-700 dark:text-gray-300 mb-3">
                  Dit is de basis van je portfolio. Je koopt aandelen of ETFs die je wilt houden voor de lange termijn.
                  Denk aan bedrijven waar je in gelooft of gediversifieerde ETFs zoals SPY, QQQ, of sector-specifieke ETFs.
                </p>
                {currentPortfolio?.hasOptions ? (
            ...
            <div className="bg-gradient-to-r from-primary-50 to-positive-50 dark:from-blue-900/10 dark:to-green-900/10 border border-primary-200 dark:border-primary-700/30 rounded-lg p-6">
              ...
              <div className="space-y-3">
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="flex-1 text-left">
                      <p className="font-medium text-gray-900 dark:text-white">LEAPS (Synthetische Aandelen)</p>
                      <p className="text-sm text-gray-600 dark:text-gray-400">
