...
import { TrendingDown, TrendingUp, Plus, ListTodo, Info, X, GraduationCap, ArrowRight, Lightbulb } from 'lucide-react';
...
export const SpreadsStrategy: React.FC = () => {
  ...
  const [strategyRules, setStrategyRules] = useState<StrategyRule[]>(() => {
    const saved = localStorage.getItem(`strategy-rules-spreads-${portfolio}`);
    if (saved) {
      return JSON.parse(saved) as StrategyRule[];
    }
    return getDefaultRulesForStrategy('spreads', portfolio || '');
  });
  ...
  useEffect(() => {
    localStorage.setItem(`strategy-rules-spreads-${portfolio}`, JSON.stringify(strategyRules));
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Van alle gesloten spreads</p>
        </div>
      </div>
              ...
              <div className="flex-1 space-y-6">
                ...
                <div className="bg-primary-50 dark:bg-primary-900/20 border border-primary-200 dark:border-primary-700/30 rounded-lg p-5">
                  ...
                  <div className="space-y-2 text-xs text-gray-700 dark:text-gray-300">
                    <p>• <strong>1 Put spread</strong> = Bull Put Spread (bullish/neutraal)</p>
                    <p>• <strong>1 Call spread</strong> = Bear Call Spread (bearish/neutraal)</p>
                    <p>• <strong>Put spread + Call spread samen</strong> = Iron Condor (neutraal)</p>
                  </div>
